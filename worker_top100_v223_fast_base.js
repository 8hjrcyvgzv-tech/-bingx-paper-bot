...
function analyze(symbol,h4,h1,m15,btcDir,quoteVolume=null,marketLastPrice=null,premium=null) {
  ...
  const dir4h=trend(h4), dir1h=trend(h1), dir15=trend(m15Use);
  ...
  const atrPct=a?(a/analysisClose)*100:99, adx4h=adx(h4,14), ema200=ema200Alignment(h4,direction);
  ...
  const recent15mBars=m15Use.slice(-6).map(r=>({time:Number(r.time||0),open:Number(r.open),high:Number(r.high),low:Number(r.low),close:Number(r.close),volume:Number(r.volume||0)}));
  ...
  return {
    ...
    atr1hPct:a1==null?null:+((a1/currentLast)*100).toFixed(2),recent15mBars,
...
async function scan(extraSymbols=[]){
  ...
  for(const item of [...top.filter(x=>coreSet.has(x.symbol)),...top.filter(x=>focusSet.has(x.symbol)),...shardItems])selectedMap.set(item.symbol,item);
  ...
  return {
    mode:"PAPER_SCAN",version:"TOP100_V2_23_FAST",scannedAt:new Date().toISOString(),btcDirection:btcDir,minScore:CFG.minScore,
    universe:`BingX top ${top.length} USDT perpetual by 24h quote volume`,shard:shardIndex+1,shardCount,
    scannedSymbols:selected.map(x=>x.symbol),focusSymbols:[...focusSet],coreFastSymbols:CORE5.filter(s=>selectedMap.has(s)),paperBalanceUSDT:CFG.paperBalance,
...
function openPaperMetrics(trade){
  ...
  const pnlUSDT=CFG.liveMarginUSDT*(gross5xPct/100);
  return {
  ...
  async notifyClosed(closedNow){
    for(const t of closedNow){
      ...
      const msg=[
        ...
        `Sonuç: ${resultLabel}`,
        `Yaklaşık sonuç: ${approxGrossPct>=0?"+":"-"}$${Math.abs(CFG.liveMarginUSDT*approxGrossPct/100).toFixed(2)}`,
