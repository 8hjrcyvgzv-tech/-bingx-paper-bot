// BingX Paper Bot V3.2 — Single-file dashboard build
// Generated from V3.2 multi-module source. Keep EXECUTION_MODE TEST.
...
function errInfo(stage,e){
  ...
  console.error(`V3.2 ${stage}`,stack||message);
...
function upgradeData(raw){
  ...
  return {...raw,version:"BINGX_WIDE_V3_2",minScore:7.5,signals:[],all};
}
...
async function scanThroughBase(env,focusSymbols=[],radarLeaders=[]){
  ...
  const data=await enrichHybridData(raw);
...
function mergeFocus(radar,watch,discovery){
  ...
  const buckets=[
    (Array.isArray(radar)?radar:[]).slice(0,6),
    (Array.isArray(watch)?watch:[]).slice(0,3),
    (Array.isArray(discovery)?discovery:[]).slice(0,3),
  ];
...
async function getWideTickerSnapshot(){
  ...
  return rows.filter(x=>typeof x?.symbol==="string"&&x.symbol.endsWith("-USDT")&&!isSyntheticSymbol(x.symbol)).map(x=>{
    ...
    const spreadPct=mid?((ask-bid)/mid)*100:null;
    return {symbol:String(x.symbol),lastPrice:last,quoteVolume,spreadPct:Number.isFinite(spreadPct)?spreadPct:null};
...
async function sendAiReviewWebhook(env,candidates,scannedAt){
  ...
  const res=await fetch(url,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({source:"BINGX_V3_2",mode:"PAPER_TEST",scannedAt,candidates})});
...
function signalMessage(x){
  return [
    ...
    `${publicStatusLabel(x.publicStatus)} · HYBRID ${x.hybridExecutionScore??x.executionScore??"-"}/10 · ${x.paperEntryType==="EARLY_STARTER"?"ERKEN STARTER":"TEYİTLİ"}`,
    ...
    `RSI4s ${x.rsi4h??"-"} · ADX4s ${x.adx4h??"-"} · 15dk Hacim ${x.volumeRatio??"-"}x · Radar ${x.v32RadarScore??"-"}/10 · Akış ${x.v32FlowPaceX??"-"}x · Funding ${x.fundingRate??"-"}`,
    `Teorik margin: ${marginForSignal(x)??"-"} USDT · 5x isolated · PAPER/TEST ONLY`,
    x.paperEntryType==="EARLY_STARTER"?`ERKEN STARTER: hacim radarı hareketi öne aldı; kırılım teyidi gelmeden küçük PAPER kademe. LIVE için temel/katalizör incelemesi zorunlu.`:`Konsensüs: Emre yön/HTF + Aksel yapı/kırılım + Belit execution aynı yönde doğrulandı.`
  ].join("\n");
}
...
function paperSummaryHtml(paper){
  ...
  const fresh=trades.filter(t=>t.scannerVersion==="V3.2");
  ...
  return `V3.2 YENİ: <b>${freshOpen.length} açık</b> · ${freshClosed.length} kapanan · Win %${freshWr.toFixed(1)} · <span class="muted">Geçmiş kayıt: ${totalOpen} açık / ${totalClosed} kapanan</span>`;
}
...
function openPaperHtml(paper){
  ...
  return trades.map(t=>{
    ...
    const meta=t.scannerVersion==="V3.2"?` · Emre ${esc(t.emreScore??"-")}/10 · Aksel ${esc(t.akselScore??"-")}/10 · Belit ${esc(t.belitScore??"-")}/10 · Hybrid ${esc(t.executionScore??t.entryQuality??"-")}/10 · ${esc(publicStatusLabel(t.publicStatus||"WAIT"))}`:"";
...
function volumeRadarHtml(data){
  const d=Array.isArray(data.volumeRadar)?data.volumeRadar:[];
  if(!d.length)return `<div class="muted">Radar ısınırken veya bu dakikada anlamlı hacim/fiyat anomalisi yok.</div>`;
  return d.map(x=>`<div class="paperCard"><b>${esc(x.symbol)} · ${esc(x.directionHint??"NEUTRAL")} · Radar ${esc(x.radarScore??"-")}/10</b><div>Akış ${esc(x.flowPaceX??"-")}x günlük-dakika temposu · kendi bazına göre ${esc(x.flowAccelX??"-")}x · +$${esc(x.flowUsd??0)}</div><div>1dk %${esc(x.price1mPct??"-")} · kısa EMA sapma %${esc(x.price5mPct??"-")} · orta EMA sapma %${esc(x.price15mPct??"-")} · spread %${esc(x.spreadPct??"-")}</div><small>${esc(x.reason??"Erken hacim/hareket anomalisi")} · Radar trade değildir; ağır Emre+Aksel+Belit analizi için öne alınır.</small></div>`).join("");
}

function page(data,paper,migration){
  const archived=Number(migration?.archivedTrades||0);
  return `<!doctype html><meta name="viewport" content="width=device-width,initial-scale=1"><title>BingX Paper Bot V3.2 Early Volume Radar</title><style>body{font-family:-apple-system,BlinkMacSystemFont,sans-serif;max-width:900px;margin:auto;padding:18px;background:#0b0d10;color:#f4f4f5}.card,.paperWrap{background:#171a1f;border:1px solid #2a2f37;border-radius:14px;padding:14px;margin:10px 0;line-height:1.55}.paperWrap{background:#111419}.paperCard{padding:10px 0;border-bottom:1px solid #2a2f37}.paperCard:last-child{border-bottom:0}.row{display:flex;justify-content:space-between;gap:12px}.good{color:#4ade80}.warn{color:#facc15}.bad{color:#fb7185}.muted,small{color:#a1a1aa}.notifyBtn{margin:10px 6px 4px 0;padding:10px 14px;border:0;border-radius:10px;font-weight:700}</style><div class="head"><h2>BingX Paper Bot · V3.2 · Early Volume Radar</h2><div><b>PAPER/TEST ONLY</b> · EXECUTION_MODE zorla TEST · BTC ${esc(data.btcDirection)}</div><div><b>1. katman — TÜM EVREN:</b> tek ticker çağrısıyla her dakika likit BingX USDT perpetual paritelerinde hacim akış hızı + kendi bazına göre hacim ivmesi + 1dk fiyat hızı + kısa/orta EMA sapması + spread anomalisi.</div><div><b>2. katman — AĞIR ANALİZ:</b> radar liderleri + mevcut ARMED/DISCOVERY adayları Emre HTF/Fib + Aksel yapı/kırılım + Belit SMA/ADR-ATR/hacim motoruna öne alınır.</div><div><b>3. katman — ERKEN STARTER:</b> gerçek kırılım beklenmeden yalnız güçlü radar + 7.75+ setup + sınır yakınlığı + Emre/Aksel/Belit kalite + risk/hedef geçerliliğinde 8.3+ küçük PAPER starter açılabilir. Normal teyitli PAPER girişler aynen devam eder.</div><div><b>Normal PAPER GİR:</b> üçlü konsensüs + gerçek breakout/retest + Hybrid Execution ≥7.5. <b>ERKEN STARTER:</b> yaklaşık 4 USDT PAPER margin; teyit gelmeden normal boyuta çıkmaz.</div><div>5x isolated · max 5 açık · aynı yön max 3 · EXECUTION_MODE TEST kilidi · gerçek emir yok.</div><div>${paperSummaryHtml(paper)}</div><div class="muted">V3.2 geçmiş işlemleri silmez. V2.x/V3.0/V3.1 kayıtları korunur; yeni V3.2 örnekleri scannerVersion ile ayrılır. AI_REVIEW_WEBHOOK_URL tanımlanırsa güçlü erken adaylar harici inceleme katmanına JSON olarak aktarılabilir; tanımlı değilse bot PAPER/TEST içinde çalışmaya devam eder.</div><button class="notifyBtn" onclick="testNotify(this)">BİLDİRİM TESTİ</button><button class="notifyBtn" onclick="testBingx(this)">BINGX BAĞLANTI TESTİ</button><div>Geniş likit evren ${esc(data.universeCount??data.volumeRadarUniverseCount??"-")} coin · Ağır analiz dilimi ${esc(data.shard)}/${esc(data.shardCount)} · Bu tur ${(data.all||[]).length} coin · HACİM RADAR ${(data.volumeRadar||[]).length} · DISCOVERY ${(data.discovery||[]).length} · HAZIR/ARMED ${(data.watch||[]).length} · PAPER GİR ${(data.signals||[]).length}</div><small>Son tarama ${esc(data.scannedAt)}</small></div><div class="paperWrap"><b>ERKEN HACİM / HAREKET RADARI — TÜM UYGUN EVREN</b>${volumeRadarHtml(data)}</div><div class="paperWrap"><b>PRE-BREAKOUT DISCOVERY RADARI</b>${discoveryHtml(data)}</div><div class="paperWrap"><b>ÜÇLÜ KONSENSÜS RADARI</b>${watchHtml(data)}</div><div class="paperWrap"><b>AÇIK AUTO PAPER POZİSYONLARI</b>${openPaperHtml(paper)}</div>${scanCards(data)}<script>async function testNotify(btn){const old=btn.textContent;btn.disabled=true;btn.textContent="Gönderiliyor...";try{const r=await fetch('/notify-test',{method:'POST'});const j=await r.json();if(!r.ok||!j.ok)throw new Error(j.error||'hata');alert('Test bildirimi gönderildi');}catch(e){alert('Bildirim testi başarısız: '+e.message);}finally{btn.disabled=false;btn.textContent=old;}}async function testBingx(btn){if(!confirm('BingX API ve TEST emir yolunu kontrol edelim mi? Gerçek pozisyon açılmaz.'))return;const old=btn.textContent;btn.disabled=true;btn.textContent='Kontrol ediliyor...';try{const r=await fetch('/bingx-connection-test',{method:'POST'});const j=await r.json();if(!r.ok||!j.ok)throw new Error(j.error||'hata');alert('BINGX TEST BAŞARILI — gerçek pozisyon açılmadı.');}catch(e){alert('BingX testi başarısız: '+e.message);}finally{btn.disabled=false;btn.textContent=old;}}</script>`;
}
...
class PaperTracker extends BasePaperTracker {
  async ensureV219Migration(){
    ...
    const m={version:"V3.2",archivedAt,archivedTrades:oldTrades.length};
  ...
  async addSignals(trades,signals,scannedAt){
    ...
    for(const t of trades){
      ...
      if(t.paperEntryType==="EARLY_STARTER")t.paperMarginUSDT=4;
    }
    return added;
  }
      ...
      if(request.method==="POST"){
        ...
        for(const r of rows){
          ...
          if(candidate){
            ...
            leaders.push({symbol:sym,radarScore:score,directionHint,flowUsd:Math.round(flowUsd),flowPaceX:+paceX.toFixed(2),flowAccelX:+accelX.toFixed(2),price1mPct:p1==null?null:+p1.toFixed(2),price5mPct:p5==null?null:+p5.toFixed(2),price15mPct:p15==null?null:+p15.toFixed(2),quoteVolume24h:Math.round(qv),spreadPct:Number.isFinite(spread)?+spread.toFixed(3):null,hotCount,reason:parts.join(" · ")||"erken hareket anomalisi",updatedAt:ts});
          }
          market[sym]={lastPrice:last,quoteVolume:qv,flowEma:nextEma,hotCount,ema5,ema15,updatedAt:ts};
        }
...
const __main_default = {
  async fetch(request,env){
    ...
    if(url.pathname==="/notify-test"){
      if(request.method!=="POST")return new Response("Method Not Allowed",{status:405});
      try{const channel=await sendPrimary(env,"BingX BİLDİRİM TESTİ · V3.2","Bildirim kanalı çalışıyor. V3.2 Early Volume Radar + Wide Discovery + Emre+Aksel+Belit aktiftir. Telegram PAPER GİR/ERKEN STARTER işlemlerini gönderir; radarın ham adayları panelde kalır. Worker EXECUTION_MODE zorla TEST kilitlidir; gerçek emir açılamaz.");return Response.json({ok:true,channel},{headers:{"cache-control":"no-store"}});}catch(e){return Response.json({ok:false,error:String(e?.message||e)},{status:500});}
    }
    ...
    if(url.pathname==="/"||url.pathname===""){
      ...
      if(!data){
        ...
        return new Response(`<!doctype html><meta name="viewport" content="width=device-width,initial-scale=1"><body style="font-family:-apple-system,BlinkMacSystemFont,sans-serif;padding:24px"><h2>BingX Paper Bot · V3.2</h2><p><b>Tarama geçici olarak durdu.</b> PAPER/TEST kilidi aktif; gerçek emir açılamaz.</p><p>Hata aşaması: <b>${esc(scanError?.stage||"SCAN")}</b><br>${esc(scanError?.message||"bilinmeyen hata")}</p>${detail}<p>Paper istatistik katmanı: ${paperError?"hata":"çalışıyor"}</p></body>`,{status:500,headers:{"content-type":"text/html; charset=UTF-8","cache-control":"no-store"}});
      }
  ...
  async scheduled(controller,env,ctx){
    ...
    // Telegram V3.2’de yalnız gerçek PAPER girişleri (teyitli veya küçük EARLY STARTER) gönderir.
    ...
    const aiReview=(data.signals||[]).filter(x=>x?.paperEntryType==="EARLY_STARTER" || (Number(x?.v32RadarScore)>=V32_RADAR.alertScore&&Number(x?.hybridSetupScore)>=7.5));
