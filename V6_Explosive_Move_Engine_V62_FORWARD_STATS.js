...
class PaperTracker extends BasePaperTracker {
  ...
  async getV6ForwardStats(){
    ...
    const avg=(xs)=>{
      ...
      return v.length?roundN(v.reduce((a,b)=>a+b,0)/v.length,3):null;
    };
    const med=(xs)=>{
      const v=xs.map(Number).filter(Number.isFinite).sort((a,b)=>a-b);
      if(!v.length)return null;
      const m=Math.floor(v.length/2);
      return roundN(v.length%2?v[m]:(v[m-1]+v[m])/2,3);
    };
    const pct=(n,d)=>d>0?roundN(100*n/d,1):null;
    const summarize=(rows,h)=>{
      const done=rows.filter(x=>x?.horizons?.[h]);
      const vals=done.map(x=>x.horizons[h]);
      ...
      return {
        samples:done.length,
        avgReturnPct:avg(rets),medianReturnPct:med(rets),
        avgMfePct:avg(mfes),medianMfePct:med(mfes),
        avgMaePct:avg(maes),medianMaePct:med(maes),
        positiveReturnPct:pct(rets.filter(x=>x>0).length,rets.length),
        return1Pct:pct(rets.filter(x=>x>=1).length,rets.length),
        return2Pct:pct(rets.filter(x=>x>=2).length,rets.length),
    ...
    const horizonSummary=(rows)=>Object.fromEntries(horizons.map(h=>[h,summarize(rows,h)]));
    const group=(name,test)=>{
      const rows=arr.filter(test);
      return {name,total:rows.length,horizons:horizonSummary(rows)};
    };
    const scoreBands=[
      ["5.5-5.99",5.5,6],["6.0-6.99",6,7],["7.0-7.99",7,8],["8.0-8.99",8,9],["9.0-10",9,10.01]
    ].map(([name,min,max])=>group(name,x=>{const v=Number(x?.preRankScore);return Number.isFinite(v)&&v>=min&&v<max;}));
    const thresholds={
      preRank:[6,7,8,9].map(t=>group(`preRank>=${t}`,x=>Number(x?.preRankScore)>=t)),
      flowPace:[1.5,2,3,5].map(t=>group(`flowPaceX>=${t}`,x=>Number(x?.flowPaceX)>=t)),
      flowAccel:[1.75,2,3,5].map(t=>group(`flowAccelX>=${t}`,x=>Number(x?.flowAccelX)>=t)),
      volExpansion:[1.4,1.8,2.5,3].map(t=>group(`volExpansionX>=${t}`,x=>Number(x?.volExpansionX)>=t)),
      bookAligned:[
        ["bookAligned>=55/<=45",55,45],
        ["bookAligned>=60/<=40",60,40],
        ["bookAligned>=65/<=35",65,35],
        ["bookAligned>=70/<=30",70,30]
      ].map(([name,longMin,shortMax])=>group(name,x=>x?.direction==="LONG"?Number(x?.bookBuyPct)>=longMin:x?.direction==="SHORT"?Number(x?.bookBuyPct)<=shortMax:false))
    };
    const combos=[
      group("rank>=7 + pace>=1.5 + accel>=1.75",x=>Number(x?.preRankScore)>=7&&Number(x?.flowPaceX)>=1.5&&Number(x?.flowAccelX)>=1.75),
      group("rank>=8 + pace>=2 + accel>=2",x=>Number(x?.preRankScore)>=8&&Number(x?.flowPaceX)>=2&&Number(x?.flowAccelX)>=2),
      group("rank>=8 + accel>=3 + vol>=1.5",x=>Number(x?.preRankScore)>=8&&Number(x?.flowAccelX)>=3&&Number(x?.volExpansionX)>=1.5),
      group("rank>=8 + accel>=3 + vol>=1.5 + alignedBook",x=>Number(x?.preRankScore)>=8&&Number(x?.flowAccelX)>=3&&Number(x?.volExpansionX)>=1.5&&(x?.direction==="LONG"?Number(x?.bookBuyPct)>=55:x?.direction==="SHORT"?Number(x?.bookBuyPct)<=45:false)),
      group("rank>=9 + accel>=3 + vol>=1.8",x=>Number(x?.preRankScore)>=9&&Number(x?.flowAccelX)>=3&&Number(x?.volExpansionX)>=1.8)
    ];
    return {
      ok:true,generatedAt:new Date().toISOString(),sampleCount:arr.length,
      note:"Forward-label analytics. preRankScore radar on-siralama skorudur; V6 Explosion Score degildir. returnPct yonlu getiridir; LONG/SHORT yonune gore isaretlidir.",
      completed:{
        30:arr.filter(x=>x?.horizons?.["30"]).length,
        120:arr.filter(x=>x?.horizons?.["120"]).length,
        360:arr.filter(x=>x?.horizons?.["360"]).length
      },
      overall:horizonSummary(arr),
      byDirection:{
        LONG:{total:arr.filter(x=>x?.direction==="LONG").length,horizons:horizonSummary(arr.filter(x=>x?.direction==="LONG"))},
        SHORT:{total:arr.filter(x=>x?.direction==="SHORT").length,horizons:horizonSummary(arr.filter(x=>x?.direction==="SHORT"))}
      },
      scoreBands,thresholds,combos
    };
  }
