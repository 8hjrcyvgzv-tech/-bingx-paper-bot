...
async function scan(){
  ...
  return {
    mode:"PAPER_ONLY",version:"TOP100_V2_2",scannedAt:new Date().toISOString(),btcDirection:btcDir,minScore:CFG.minScore,
...
async function sendNtfy(env,data){
  ...
  const body=data.signals.map(x=>[
    ...
    `Runner: kalan %${x.runnerPct} · TP1 sonrası BE · TP2 sonrası 1s EMA20/${x.runnerTrailATR??"-"} ATR trailing`,
    `Brüt 5x teorik: TP1 ≈ %${x.grossTp1Pct??"-"} · TP2 ≈ %${x.grossTp2Pct??"-"} (ücret/slippage hariç)`,
    `4s/1s/15dk: ${x.trend4h}/${x.trend1h}/${x.trend15m}`,
    `Hacim: ${x.volumeRatio??"-"}x | Alış baskısı: ${x.buyPressurePct??"-"}%`,
  ...
  const res=await fetch(`https://ntfy.sh/${encodeURIComponent(env.NTFY_TOPIC)}`,{
    method:"POST",
    headers:{"content-type":"text/plain; charset=utf-8",title:"BingX 7/10+ Sinyal · Top100 V2.2",priority:"high",tags:"chart_with_upwards_trend"},
    body,
  });
...
function page(data){
  const cards=data.all.map(x=>{
    ...
    <div>Runner: TP1 sonrası SL girişe · TP2 sonrası 1s EMA20 / ${x.runnerTrailATR??"-"} ATR trailing</div>
  ...
  <div class="head"><h2>BingX Paper Bot · Top100 V2.2</h2><div>Mod: PAPER ONLY · Eşik: ${data.minScore}/10 · BTC: ${data.btcDirection}</div>
  <div>4s + 1s + 15dk · Dinamik 1s hedef · 2R/3-5R · %50 Runner · EMA200 + ADX + baskı*</div>
