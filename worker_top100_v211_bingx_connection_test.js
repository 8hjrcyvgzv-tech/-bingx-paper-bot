...
async function scan(){
  ...
  return {
    mode:"PAPER_SCAN",version:"TOP100_V2_11",scannedAt:new Date().toISOString(),btcDirection:btcDir,minScore:CFG.minScore,
...
async function bingxConnectionTest(env){
  // Bu fonksiyon EXECUTION_MODE ne olursa olsun SADECE BingX /order/test kullanır.
  // Gerçek emir oluşturmaz; margin type veya leverage ayarını da değiştirmez.
  if(!env?.BINGX_API_KEY||!env?.BINGX_SECRET_KEY)throw new Error("BingX API anahtarları eksik");

  const symbol="BTC-USDT";
  const [positions,balanceData,dualData,contract,m15]=await Promise.all([
  ...
  const available=Number(usdt?.availableMargin??usdt?.availableBalance??usdt?.balance??0);
  ...
  // Test endpoint'i için geçerli olacak kadar büyük, fakat yalnızca sanal bir miktar.
  ...
  const active=(Array.isArray(positions)?positions:[]).filter(x=>Math.abs(Number(x?.positionAmt||0))>0).length;
  ...
  return {
    ...
    availableUSDT:Number.isFinite(available)?+available.toFixed(2):null,
...
async function sendSignalAlerts(env,data){
  ...
  for(const x of data.signals){
    ...
    const message=[
      ...
      `${(x.reasons||[]).join(" · ")}`,
      token?`İşlem bağlantısı ${Math.round(CFG.approvalTtlMs/60000)} dk geçerli; açınca sinyal yeniden kontrol edilir.`:"İşlem bağlantısı oluşturulamadı."
    ].join("\n");

    const actionUrl=token?`${base}/signal?token=${encodeURIComponent(token)}`:base;
    const actionLabel=token
      ? (execMode==="LIVE"?`${x.direction} AÇ`:`${x.direction} TEST`)
      : "Bot sitesini aç";

    try{
      await sendPrimary(env,`BingX ${x.direction} ${x.score}/10 · V2.11`,message,actionUrl,actionLabel);
    }catch(e){
...
function page(data,execMode="KAPALI"){
  ...
  <div class="head"><h2>BingX Paper Bot · Top100 V2.11</h2><div>Tarama: PAPER · İşlem köprüsü: ${execMode} · Eşik: ${data.minScore}/10 · BTC: ${data.btcDirection}</div>
