...
async function scan(){
  ...
  return {
    mode:"PAPER_SCAN",version:"TOP100_V2_12",scannedAt:new Date().toISOString(),btcDirection:btcDir,minScore:CFG.minScore,
...
async function sendSignalAlerts(env,data){
  ...
  for(const x of data.signals){
    ...
    try{
      await sendPrimary(env,`BingX ${x.direction} ${x.score}/10 · V2.12`,message,actionUrl,actionLabel);
    }catch(e){
...
function page(data,execMode="KAPALI"){
  ...
  <div class="head"><h2>BingX Paper Bot · Top100 V2.12</h2><div>Tarama: PAPER · İşlem köprüsü: ${execMode} · Eşik: ${data.minScore}/10 · BTC: ${data.btcDirection}</div>
  <div>4s + 1s + 15dk · 5x isolated · 20 USDT margin · LIVE max 5 pozisyon · TEST limitsiz · tek tuş onay</div>
