...
import { enrichBelitData, isV220Signal, stageLabel } from "./belit_daily_v220.js";
...
function signalMessage(x){
  return [
    ...
    `${stageLabel(x.belitStage)} · Setup ${x.setupQuality??"-"}/10 · Giriş ${x.entryQuality??"-"}/10 · Klasik ${x.score}/10`,
    `Giriş: ${x.entry}`,
    `SL: ${x.stop??"-"} (${x.stopPct??"-"}%)`,
    `TP1: ${x.tp1??"-"} · TP2: ${x.tp2??"-"}`,
    `Sınır: ${x.boundary??"-"} · test ${x.boundaryTests??0} · mesafe ${x.distanceToBoundaryPct??"-"}%`,
    `1G ADR20 %${x.adr20Pct??"-"} · ATR20 %${x.atr20DailyPct??"-"} · sıkışma ${x.compressionRatio??"-"}x · SMA ${x.smaDaily??"-"}`,
    `4s/1s/15dk: ${x.trend4h}/${x.trend1h}/${x.trend15m} · ADX4s ${x.adx4h??"-"} · Hacim ${x.volumeRatio??"-"}x`,
    `AUTO PAPER: taramadaki en iyi 1-2 girişten biri; uzamış hareket kovalanmadı.`
  ].join("\n");
}
...
function watchMessage(x){
  return [
    `${x.symbol} ${x.direction} · ${x.isCore5?"CORE5":"GENİŞ TARAMA"}`,
    `${stageLabel(x.belitStage)} · Setup ${x.setupQuality??"-"}/10 · ŞİMDİ GİRİŞ YOK`,
    `Ana sınır: ${x.boundary??"-"} · ${x.boundaryTests??0} test · mesafe ${x.distanceToBoundaryPct??"-"}%`,
    `Sıkışma ${x.compressionRatio??"-"}x · yönlü sıkışma ${x.directionalSqueeze?"EVET":"-"}`,
    `1G SMA ${x.smaDaily??"-"} · ADR20 %${x.adr20Pct??"-"} · ATR20 %${x.atr20DailyPct??"-"}`,
    `Tetik: sınır üstü/altı kalıcı mum + hacim. Kırılım uzarsa KOVALAMA; retest bekle.`
  ].join("\n");
}
...
function paperSummaryHtml(paper){
  ...
  return `AUTO PAPER V2.20: <b>${open} açık</b> · ${closed} kapanan · Win %${wr.toFixed(1)} · Açık toplam <span class="${pnl>0?"good":pnl<0?"bad":"muted"}">${pnl>=0?"+":"-"}$${Math.abs(pnl).toFixed(2)}</span>`;
}
...
function openPaperHtml(paper){
  ...
  return trades.map(t=>{
    ...
    const meta=t.scannerVersion==="V2.20"?` · Setup ${esc(t.setupQuality??"-")}/10 · Giriş ${esc(t.entryQuality??"-")}/10 · ${esc(stageLabel(t.belitStage||"NONE"))}`:"";
...
function scanCards(data){
  return (data.all||[]).map(x=>{
    ...
    const ok=Boolean(x.v220Qualifies), watch=Boolean(x.belitWatchCandidate);
    const cls=ok?"good":watch?"warn":"muted";
    const label=ok?"PAPER GİR":watch?"ERKEN UYARI":x.belitExtended?"KAÇMIŞ / RETEST BEKLE":"BEKLE/PAS";
    const baseReject=(x.v219RejectReasons||[]).join(" · ")||"filtre";
    const reason=x.belitExtended?"ADR/ATR açısından uzamış; kovalanmaz":x.v219Qualifies&&Number(x.entryQuality)<7?"giriş kalitesi<7":baseReject;
    return `<div class="card"><div class="row"><b>${esc(x.symbol)} ${x.isCore5?"· CORE5":"· GENİŞ"}</b><span class="${cls}">${esc(label)}</span></div><div>${esc(stageLabel(x.belitStage||"NONE"))} · Setup <b>${esc(x.setupQuality??"-")}/10</b> · Giriş <b>${esc(x.entryQuality??"-")}/10</b> · Klasik ${esc(x.score)}/10</div><div>Sınır ${esc(x.boundary??"-")} · test ${esc(x.boundaryTests??0)} · mesafe ${esc(x.distanceToBoundaryPct??"-")}% · kalıcılık ${x.persistentBreakout?"✅":"—"}</div><div>1G SMA ${esc(x.smaDaily??"-")} · ADR20 %${esc(x.adr20Pct??"-")} · ATR20 %${esc(x.atr20DailyPct??"-")} · sıkışma ${esc(x.compressionRatio??"-")}x</div><div>4s/1s/15dk ${esc(x.trend4h)} / ${esc(x.trend1h)} / ${esc(x.trend15m)} · EMA200 ${x.ema200Aligned?"✅":"—"} · ADX ${esc(x.adx4h??"-")} · Hacim ${esc(x.volumeRatio??"-")}x</div><div>Giriş ${esc(x.entry)} · SL ${esc(x.stop??"-")} · TP1 ${esc(x.tp1??"-")} · TP2 ${esc(x.tp2??"-")}</div><small>${ok?"V2.20 PAPER GİR":watch?"Kırılımı bekle: kalıcı mum + hacim teyidi":`Elendi: ${esc(reason)}`}</small>${x.belitError?`<small class="bad"> · 1G analiz: ${esc(x.belitError)}</small>`:""}</div>`;
  }).join("");
}

function watchHtml(data){
  const w=Array.isArray(data.watch)?data.watch:[];
  if(!w.length)return `<div class="muted">Bu turda 7.5/10+ erken breakout hazırlığı yok.</div>`;
  return w.map(x=>`<div class="paperCard"><b>${esc(x.symbol)} · ${esc(stageLabel(x.belitStage))} · Setup ${esc(x.setupQuality)}/10</b><div>Sınır ${esc(x.boundary??"-")} · ${esc(x.boundaryTests??0)} test · mesafe ${esc(x.distanceToBoundaryPct??"-")}% · ŞİMDİ GİRİŞ YOK</div></div>`).join("");
}

function page(data,paper,migration){
  const archived=Number(migration?.archivedTrades||0);
  return `<!doctype html><meta name="viewport" content="width=device-width,initial-scale=1"><title>BingX Paper Bot V2.20</title><style>body{font-family:-apple-system,BlinkMacSystemFont,sans-serif;max-width:820px;margin:auto;padding:18px;background:#0b0d10;color:#f4f4f5}.card,.paperWrap{background:#171a1f;border:1px solid #2a2f37;border-radius:14px;padding:14px;margin:10px 0;line-height:1.55}.paperWrap{background:#111419}.paperCard{padding:10px 0;border-bottom:1px solid #2a2f37}.paperCard:last-child{border-bottom:0}.row{display:flex;justify-content:space-between;gap:12px}.good{color:#4ade80}.warn{color:#facc15}.bad{color:#fb7185}.muted,small{color:#a1a1aa}.notifyBtn{margin:10px 6px 4px 0;padding:10px 14px;border:0;border-radius:10px;font-weight:700}</style><div class="head"><h2>BingX Paper Bot · Top100 V2.20 · Belit Breakout</h2><div>Tarama: PAPER · klasik kapı <b>8/10+</b> + ayrı <b>Setup/Giriş kalitesi</b> · BTC ${esc(data.btcDirection)}</div><div>Yeni: 1G yatay sınır/test · pre-breakout sıkışma · kalıcı breakout · retest · continuation · SMA10/20/50/100/200 · ADR/ATR · KAÇMIŞ filtresi</div><div>4s+1s ana trend · 15dk tetik · EMA200 · ADX≥22 · hacim≥1.05x · baskı≥%55 · tur başına en iyi 1-2 PAPER giriş</div><div>5x isolated · 20 USDT margin · AUTO PAPER max 5 açık · aynı yönde max 3</div><div>${paperSummaryHtml(paper)}</div><div class="muted">V2.18 arşivi ${archived} işlem. V2.20 mevcut paper kayıtlarını korur; yeni işlemlere Setup/Giriş etiketi ekler.</div><button class="notifyBtn" onclick="testNotify(this)">BİLDİRİM TESTİ</button><button class="notifyBtn" onclick="testBingx(this)">BINGX BAĞLANTI TESTİ</button><div>Top100 likit evren · Dilim ${esc(data.shard)}/${esc(data.shardCount)} · Bu tur ${(data.all||[]).length} coin · PAPER GİR ${(data.signals||[]).length} · ERKEN UYARI ${(data.watch||[]).length}</div><small>Son tarama ${esc(data.scannedAt)}</small></div><div class="paperWrap"><b>ERKEN BREAKOUT RADARI</b>${watchHtml(data)}</div><div class="paperWrap"><b>AÇIK AUTO PAPER POZİSYONLARI</b>${openPaperHtml(paper)}</div>${scanCards(data)}<script>async function testNotify(btn){const old=btn.textContent;btn.disabled=true;btn.textContent="Gönderiliyor...";try{const r=await fetch('/notify-test',{method:'POST'});const j=await r.json();if(!r.ok||!j.ok)throw new Error(j.error||'hata');alert('Test bildirimi gönderildi');}catch(e){alert('Bildirim testi başarısız: '+e.message);}finally{btn.disabled=false;btn.textContent=old;}}async function testBingx(btn){if(!confirm('BingX API ve TEST emir yolunu kontrol edelim mi? Gerçek pozisyon açılmaz.'))return;const old=btn.textContent;btn.disabled=true;btn.textContent='Kontrol ediliyor...';try{const r=await fetch('/bingx-connection-test',{method:'POST'});const j=await r.json();if(!r.ok||!j.ok)throw new Error(j.error||'hata');alert('BINGX TEST BAŞARILI — gerçek pozisyon açılmadı.');}catch(e){alert('BingX testi başarısız: '+e.message);}finally{btn.disabled=false;btn.textContent=old;}}</script>`;
}
...
export class PaperTracker extends BasePaperTracker {
  async ensureV219Migration(){
    ...
    const m={version:"V2.20",archivedAt,archivedTrades:oldTrades.length};
...
export default {
  async fetch(request,env){
    ...
    if(url.pathname==="/notify-test"){
      ...
      try{const channel=await sendPrimary(env,"BingX BİLDİRİM TESTİ · V2.20","Bildirim kanalı çalışıyor. V2.20 Belit breakout radarı; erken uyarı, Setup/Giriş kalitesi ve KAÇMIŞ filtresi aktif.");return Response.json({ok:true,channel},{headers:{"cache-control":"no-store"}});}catch(e){return Response.json({ok:false,error:String(e?.message||e)},{status:500});}
    }
  ...
  async scheduled(controller,env,ctx){
    ...
    const [alerts,watchAlerts]=await Promise.all([
      claimAlerts(env,data.signals||[],data.scannedAt),
      claimWatch(env,data.watch||[],data.scannedAt)
    ]);
    console.log(JSON.stringify({cron:controller.cron,version:"V2.20",scannedAt:data.scannedAt,shard:data.shard,scanned:data.all?.length||0,qualified:data.signals?.length||0,watch:data.watch?.length||0,alerts:alerts.length,watchAlerts:watchAlerts.length,paper:paper?.stats||null}));
    if(alerts.length||watchAlerts.length){
      ctx.waitUntil((async()=>{
        for(const x of watchAlerts){
          try{await sendPrimary(env,`BingX ERKEN UYARI · ${x.symbol} · Setup ${x.setupQuality}/10`,watchMessage(x));}catch(e){console.error("WATCH_ALERT_ERROR",String(e?.message||e));}
          await sleep(900);
        }
        for(const x of alerts){
          try{await sendPrimary(env,`BingX PAPER GİR · ${x.symbol} ${x.direction} · Giriş ${x.entryQuality}/10`,signalMessage(x));}catch(e){console.error("ALERT_ERROR",String(e?.message||e));}
