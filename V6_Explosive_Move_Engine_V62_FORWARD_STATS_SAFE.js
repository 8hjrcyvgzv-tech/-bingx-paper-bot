// BingX Paper Bot V6.2 — Explosive Move Engine · Pulse-Risk / Early-Entry fix
...
const __base_default = {
  async fetch(request,env){
    ...
    try{
      const focus=(url.searchParams.get("focus")||"").split(",").map(x=>x.trim()).filter(Boolean).slice(0,20);
...
const V32_RADAR = {
  ...
  maxLeaders: 40,
  focusCap: 20,
  realVolumeProbeCap: 16,
  realVolumeProbeMinScore: 3.0,
...
const V6 = {
  version: "V6.2",
  maxExplosionStopPct: 3.2,
  minExplosionStopPct: 0.55,
  pulseFreshMs: 4 * 60 * 1000,
  radarFreshMs: 3 * 60 * 1000,
  minPaperScore: 7.5,
  minExpectedMoveR: 1.8,
  ignitionMinScore: 7.8,
  ignitionMinMoveR: 1.25,
  ignitionMinEarlyness: 0.65,
  ignitionMinRealVol1mX: 2.8,
  ignitionMinRealVol3mX: 1.6,
  ignitionMaxStopPct: 2.75,
  minEarlyness: 0.42,
  maxSignalsPerScan: 2,
  oiFocusCap: 20,
  forwardLabelTopN: 16,
...
function errInfo(stage,e){
  ...
  console.error(`V6.2 ${stage}`,stack||message);
...
async function fetchRealVolumePulse(symbol){
  try{
    const u=new URL("https://open-api.bingx.com/openApi/swap/v3/quote/klines");
    u.searchParams.set("symbol",symbol);
    ...
    const bars=raw.map(r=>Array.isArray(r)
      ? {time:Number(r[0]),open:Number(r[1]),high:Number(r[2]),low:Number(r[3]),close:Number(r[4]),volume:Number(r[5])}
      : {time:Number(r?.time??r?.openTime??0),open:Number(r?.open),high:Number(r?.high),low:Number(r?.low),close:Number(r?.close),volume:Number(r?.volume??r?.vol)}
    ).filter(x=>x.close>0&&x.volume>=0).sort((a,b)=>a.time-b.time);
    if(bars.length<12)return null;
    const last=bars.at(-1),prev=bars.slice(0,-1);
    const base=median(prev.slice(-20).map(x=>x.volume));
    if(!(base>0))return null;
    const sum=(n)=>bars.slice(-n).reduce((a,x)=>a+Number(x.volume||0),0);
    const p=(n)=>{const b=bars.at(-(n+1));return b?.close>0?100*(last.close-b.close)/b.close:null;};
    const last3=bars.slice(-3),last5=bars.slice(-5);
    return {
      symbol,
      realLastPrice:Number(last.close),
      realLastTime:Number(last.time)||Date.now(),
      realLow3m:Math.min(...last3.map(x=>Number(x.low))),
      realHigh3m:Math.max(...last3.map(x=>Number(x.high))),
      realLow5m:Math.min(...last5.map(x=>Number(x.low))),
      realHigh5m:Math.max(...last5.map(x=>Number(x.high))),
      realVol1mX:Math.max(Number(last.volume||0),Number(prev.at(-1)?.volume||0))/base,
      realVol3mX:sum(3)/(base*3),
      realVol5mX:sum(5)/(base*5),
      realPrice1mPct:p(1),
      realPrice3mPct:p(3),
      realPrice5mPct:p(5),
      realVolumeBase:base
...
function enrichRadarLeaderWithRealVolume(x,p){
  if(!p)return x;
  let bonus=0;
  const v1=Number(p.realVol1mX),v3=Number(p.realVol3mX),v5=Number(p.realVol5mX);
  if(v1>=5)bonus+=1.75; else if(v1>=3)bonus+=1.25; else if(v1>=2)bonus+=0.8; else if(v1>=1.4)bonus+=0.4;
  if(v3>=3)bonus+=1.15; else if(v3>=2)bonus+=0.75; else if(v3>=1.4)bonus+=0.35;
  if(v5>=2.5)bonus+=0.75; else if(v5>=1.7)bonus+=0.45; else if(v5>=1.3)bonus+=0.2;
  const rp1=Number(p.realPrice1mPct),rp5=Number(p.realPrice5mPct);
  let directionHint=x.directionHint;
  if(Number.isFinite(rp5)&&Math.abs(rp5)>=0.20)directionHint=rp5>0?"LONG":"SHORT";
  else if(Number.isFinite(rp1)&&Math.abs(rp1)>=0.10)directionHint=rp1>0?"LONG":"SHORT";
  const score=Math.max(0,Math.min(10,(Number(x.radarScore)||0)+bonus));
  const reason=[x.reason,`gerçek 1dk hacim ${Number.isFinite(v1)?v1.toFixed(2):"-"}x`,`3dk ${Number.isFinite(v3)?v3.toFixed(2):"-"}x`,`5dk ${Number.isFinite(v5)?v5.toFixed(2):"-"}x`].filter(Boolean).join(" · ");
  return {...x,...p,radarScore:Math.round(score*4)/4,directionHint,realVolumeBonus:+bonus.toFixed(2),reason};
}

async function persistRadarEnrichment(env,leaders,universeCount,scannedAt){
  try{
    await trackerStub(env).fetch("https://paper.local/v32-volume-radar",{
      method:"POST",headers:{"content-type":"application/json"},
      body:JSON.stringify({enrichOnly:true,leaders,universeCount,scannedAt})
    });
  }catch{}
}
...
async function prepareRadar(env){
  ...
  const state=await updateVolumeRadar(env,rows,scannedAt);
  const cheap=(Array.isArray(state?.leaders)?state.leaders:[]);
  const probe=cheap.filter(x=>Number(x?.radarScore)>=V32_RADAR.realVolumeProbeMinScore).slice(0,V32_RADAR.realVolumeProbeCap);
  const pulses=(await Promise.all(probe.map(x=>fetchRealVolumePulse(x.symbol)))).filter(Boolean);
  const pmap=new Map(pulses.map(x=>[x.symbol,x]));
  const leaders=cheap.map(x=>enrichRadarLeaderWithRealVolume(x,pmap.get(x.symbol)))
    .sort((a,b)=>(Number(b.radarScore)||0)-(Number(a.radarScore)||0)||(Number(b.realVolumeBonus)||0)-(Number(a.realVolumeBonus)||0)||(Number(b.flowAccelX)||0)-(Number(a.flowAccelX)||0));
  await persistRadarEnrichment(env,leaders,rows.length,scannedAt);
  return {...state,leaders,universeCount:rows.length,realVolumeProbed:pulses.length};
}
...
function earlynessScore(r,x){
  const p1=Math.abs(Number(r?.realPrice1mPct??r?.price1mPct)||0),p5=Math.abs(Number(r?.realPrice5mPct??r?.price5mPct)||0);
  const hot=Number(r?.hotCount||0),volx=Number(r?.volExpansionX);
  const realVol=Math.max(Number(r?.realVol1mX)||0,Number(r?.realVol3mX)||0);
  let e=0.74;
  if(hot<=2)e+=0.10; else if(hot>=8)e-=0.16; else if(hot>=6)e-=0.08;
  if(p5<=1.0)e+=0.10; else if(p5>=5.0)e-=0.22; else if(p5>=3.0)e-=0.10;
  if(p1>=0.10&&p1<=0.75)e+=0.07; else if(p1>=1.4)e-=0.10;
  if(Number.isFinite(volx)&&volx>=1.8)e+=0.06;
  if(realVol>=2.5)e+=0.08; else if(realVol>=1.6)e+=0.04;
  // Legacy 'extended' is now a soft penalty, not a veto. Explosions can legitimately extend.
  if(x?.belitExtended||x?.akselFlag?.extended)e-=0.12;
  if(x?.entryStillValid===false)e-=0.28;
  return clamp01(e);
}

function v6RiskPlan(x,direction,explosionScore,r={}){
  const candidates=[r?.realLastPrice,r?.lastPrice,x?.lastPrice,x?.markPrice,x?.price,x?.entry].map(Number).filter(v=>Number.isFinite(v)&&v>0);
  const entry=candidates[0];
  if(!(entry>0)||!["LONG","SHORT"].includes(direction))return {valid:false,reason:"fiyat/yön yok"};

  const pulseTime=Number(r?.realLastTime||0);
  const pulseFresh=Number.isFinite(pulseTime)&&pulseTime>0&&(Date.now()-pulseTime)<=V6.pulseFreshMs;
  const radarTime=Number(r?.updatedAt||0);
  const radarFresh=Number.isFinite(radarTime)&&radarTime>0&&(Date.now()-radarTime)<=V6.radarFreshMs;
  const bars=(Array.isArray(x?.recent15mBars)?x.recent15mBars:[]).filter(b=>Number(b?.high)>0&&Number(b?.low)>0).slice(-8);
  const atrPct=Math.abs(Number(x?.atr1hPct));
  const atrAbs=Number.isFinite(atrPct)&&atrPct>0?entry*atrPct/100:null;
  const maxStopDist=entry*V6.maxExplosionStopPct/100;
  const minStopDist=entry*V6.minExplosionStopPct/100;
  const buffer=Number.isFinite(atrAbs)?Math.min(entry*0.004,0.10*atrAbs):entry*0.0015;

  const microExtreme=direction==="LONG"
    ? Number(r?.realLow3m??r?.realLow5m)
    : Number(r?.realHigh3m??r?.realHigh5m);
  const microRisk=Number.isFinite(microExtreme)&&microExtreme>0
    ? (direction==="LONG"?entry-microExtreme:microExtreme-entry)
    : null;
  const recentExtreme=bars.length
    ? (direction==="LONG"?Math.min(...bars.slice(-5).map(b=>Number(b.low))):Math.max(...bars.slice(-5).map(b=>Number(b.high))))
    : null;
  const structuralRisk=Number.isFinite(recentExtreme)
    ? (direction==="LONG"?entry-recentExtreme:recentExtreme-entry)
    : null;
  const atrRisk=Number.isFinite(atrAbs)?0.55*atrAbs:entry*0.009;

  if(pulseFresh&&Number.isFinite(microRisk)&&microRisk>maxStopDist){
    const microStopPct=100*microRisk/entry;
    return {valid:false,reason:`3dk mikro stop ${microStopPct.toFixed(2)}%>${V6.maxExplosionStopPct}%`,entry,stopPct:microStopPct,pulseFresh,radarFresh};
  }

  let source="V6_2_ATR";
  let chosenStructure=null;
  if(pulseFresh&&Number.isFinite(microRisk)&&microRisk>0&&microRisk<=maxStopDist){
    chosenStructure=microRisk+buffer; source="V6_2_PULSE_MICRO_ATR";
  }else if(Number.isFinite(structuralRisk)&&structuralRisk>0&&structuralRisk<=maxStopDist){
    chosenStructure=structuralRisk+buffer; source="V6_2_15M_ATR";
  }

  let riskDist=Math.max(minStopDist,Math.min(maxStopDist,atrRisk),Number(chosenStructure)||0);
  // If the only available structure is already wider than the explosion risk budget,
  // do not chase it. A fresh 1m pulse is allowed to use micro-structure instead.
  if(!(riskDist>0)||riskDist>maxStopDist+1e-12)return {valid:false,reason:`stop bütçesi>${V6.maxExplosionStopPct}%`,entry,pulseFresh,radarFresh};
  const stopPct=100*riskDist/entry;
  if(stopPct>V6.maxExplosionStopPct+1e-9)return {valid:false,reason:`stop ${stopPct.toFixed(2)}%`,entry,stopPct,pulseFresh,radarFresh};

  const targetR=Number(explosionScore)>=9?4:Number(explosionScore)>=8.4?3.5:3;
  const stop=direction==="LONG"?entry-riskDist:entry+riskDist;
  const tp1=direction==="LONG"?entry+1.5*riskDist:entry-1.5*riskDist;
  const tp2=direction==="LONG"?entry+targetR*riskDist:entry-targetR*riskDist;
  return {
    valid:true,source,entry,stop,tp1,tp2,targetR,stopPct,direction,pulseFresh,radarFresh,
    tp1MovePct:1.5*stopPct,tp2MovePct:targetR*stopPct,
    riskOk:true,rangeOk:true,runnerTrailATR:Number(explosionScore)>=8.4?1.9:1.6
  };
}
function expectedMoveR(x,explosionScore,riskPlan,r){
  const stop=Math.abs(Number(riskPlan?.stopPct??x?.stopPct));
  if(!(stop>0))return null;
  const direction=riskPlan?.direction||directionFromRadar(r,x);
  const atr=Math.abs(Number(x?.atr1hPct));
  const adr=Math.abs(Number(x?.adr20Pct));
  const aligned=(v)=>{const n=directionalPct(direction,Number(v));return Number.isFinite(n)?Math.max(0,n):null;};
  const p1=aligned(r?.realPrice1mPct??r?.price1mPct);
  const p3=aligned(r?.realPrice3mPct);
  const p5=aligned(r?.realPrice5mPct??r?.price5mPct);
  const rv=Math.max(Number(r?.realVol1mX)||0,Number(r?.realVol3mX)||0,1);
  const volBoost=1+0.08*Math.min(4,Math.max(0,rv-1));
  const scoreBoost=1+0.06*Math.max(0,Number(explosionScore)-7.5);
  const candidates=[
    Number.isFinite(atr)?atr*(1.15+0.10*Math.max(0,Number(explosionScore)-7)+0.08*Math.min(4,rv-1)):null,
    Number.isFinite(adr)?adr*0.38:null,
    Number.isFinite(p1)&&rv>=1.4?p1*3.2*volBoost*scoreBoost:null,
    Number.isFinite(p3)&&rv>=1.3?p3*1.85*volBoost*scoreBoost:null,
    Number.isFinite(p5)?p5*1.35*volBoost*scoreBoost:null
  ].filter(v=>Number.isFinite(v)&&v>=0);
  if(!candidates.length)return null;
  return Math.min(6,Math.max(...candidates)/stop);
}
...
function v6SignalEligible(x){
  const ignition=x?.paperEntryType==="V6_IGNITION";
  const moveFloor=ignition?V6.ignitionMinMoveR:V6.minExpectedMoveR;
  const earlyFloor=ignition?V6.ignitionMinEarlyness:V6.minEarlyness;
  const scoreFloor=ignition?V6.ignitionMinScore:V6.minPaperScore;
  return Boolean(
    x?.v6PaperEligible &&
    x?.qualifies &&
    ["LONG","SHORT"].includes(x?.direction) &&
    Number(x?.v6ExplosionScore)>=scoreFloor &&
    Number(x?.v6ExpectedMoveR)>=moveFloor &&
    Number(x?.v6Earlyness)>=earlyFloor &&
...
async function applyV6ExplosionEngine(data,radarLeaders,env){
  ...
  const scored=all.map(x=>{
    ...
    const p1=Number(r?.realPrice1mPct??r?.price1mPct),p5=Number(r?.realPrice5mPct??r?.price5mPct);
    const momentum=weighted01([
      {weight:0.45,value:absComponent(p1,0.05,0.65)},
      {weight:0.55,value:absComponent(p5,0.15,1.8)}
    ]);
    const realVolume=weighted01([
      {weight:0.50,value:paceComponent(r?.realVol1mX,1.15,5)},
      {weight:0.30,value:paceComponent(r?.realVol3mX,1.10,3.5)},
      {weight:0.20,value:paceComponent(r?.realVol5mX,1.05,2.8)}
    ]);
    const flow=weighted01([
      {weight:0.34,value:paceComponent(r?.flowPaceX,1.15,5)},
      {weight:0.26,value:paceComponent(r?.flowAccelX,1.15,4)},
      {weight:0.10,value:absComponent(r?.flowUsd,5_000,150_000)},
      {weight:0.30,value:realVolume}
    ]);
    ...
    const raw=10*weighted01([
      ...
      {weight:0.16,value:flow},
      {weight:0.06,value:realVolume},
    ...
    const riskPlan=v6RiskPlan(x,direction,score,r);
    const moveR=expectedMoveR(x,score,riskPlan,r);
    const standardMargin=v6Margin(score,moveR,early);
    // V6.2: a fresh 1m pulse can override stale legacy entry/mark state.
    // The hard gate is a bounded explosion risk plan plus either fresh pulse price or valid legacy mark.
    const pulseFresh=Boolean(riskPlan?.pulseFresh);
    const radarFresh=Boolean(riskPlan?.radarFresh);
    const entryFresh=pulseFresh||radarFresh||x?.entryStillValid!==false;
    const safeRiskPlan=Boolean(riskPlan?.valid)&&entryFresh;
    ...
    // PAPER-only ignition lane: do not wait for the full 1.8R model when the move is
    // demonstrably early, volume is exploding, direction has momentum and the stop is tight.
    // This is deliberately a small-size learning lane; EXECUTION_MODE remains TEST.
    const ignitionVolume=Number(r?.realVol1mX)>=V6.ignitionMinRealVol1mX||Number(r?.realVol3mX)>=V6.ignitionMinRealVol3mX;
    const ignitionEligible=Boolean(
      safeRiskPlan && (pulseFresh||radarFresh) &&
      score>=V6.ignitionMinScore &&
      early>=V6.ignitionMinEarlyness &&
      Number(moveR)>=V6.ignitionMinMoveR &&
      Number(riskPlan?.stopPct)<=V6.ignitionMaxStopPct &&
      ignitionVolume && Number(momentum)>=0.45
    );
    const margin=standardMargin??(ignitionEligible?4:null);
    const paperEligible=standardEligible||ignitionEligible;
    const reject=[];
    if(!riskPlan?.valid)reject.push(`V6 risk plan: ${riskPlan?.reason||"geçersiz"}`);
    if(x?.entryStillValid===false&&!pulseFresh&&!radarFresh)reject.push("giriş/mark güncelliği geçersiz");
    if(x?.entryStillValid===false&&(pulseFresh||radarFresh))reject.push("legacy giriş stale; taze radar/pulse fiyatı ile override (veto değil)");
    if(score<V6.minPaperScore)reject.push(`explosion<${V6.minPaperScore}`);
    if(!(Number(moveR)>=V6.minExpectedMoveR)&&!ignitionEligible)reject.push(`beklenen hareket<${V6.minExpectedMoveR}R`);
    if(ignitionEligible&&!standardEligible)reject.push(`V6.2 PAPER ignition lane: ${roundN(moveR,2)}R + güçlü taze hacim (veto değil)`);
    if(early<V6.minEarlyness)reject.push(`earlyness<${V6.minEarlyness}`);
    if(x?.belitExtended||x?.akselFlag?.extended)reject.push("legacy extended (soft penalty)");
    if(!alignedBase)reject.push("legacy yön farklı (artık veto değil)");
    ...
    return {
      ...x,
      direction,
      entry:riskPlan?.valid?roundN(riskPlan.entry,8):x.entry,
      stop:riskPlan?.valid?roundN(riskPlan.stop,8):x.stop,
      tp1:riskPlan?.valid?roundN(riskPlan.tp1,8):x.tp1,
      tp2:riskPlan?.valid?roundN(riskPlan.tp2,8):x.tp2,
      stopPct:riskPlan?.valid?roundN(riskPlan.stopPct,2):x.stopPct,
      tp1MovePct:riskPlan?.valid?roundN(riskPlan.tp1MovePct,2):x.tp1MovePct,
      tp2MovePct:riskPlan?.valid?roundN(riskPlan.tp2MovePct,2):x.tp2MovePct,
      targetR:riskPlan?.valid?riskPlan.targetR:x.targetR,
      runnerTrailATR:riskPlan?.valid?riskPlan.runnerTrailATR:x.runnerTrailATR,
      rangeOk:riskPlan?.valid?true:x.rangeOk,
      riskOk:riskPlan?.valid?true:x.riskOk,
      ...
      paperEntryType:ignitionEligible&&!standardEligible?"V6_IGNITION":"V6_EXPLOSIVE",
      scannerVersion:"V6.2",
      ...
      v6StandardEligible:standardEligible,
      v6IgnitionEligible:ignitionEligible,
      v6DirectionAlignedRiskPlan:alignedBase,
      v6RiskPlanSource:riskPlan?.source||null,
      v6PulseFresh:pulseFresh,
      v6RadarFresh:radarFresh,
      v6RealLastPrice:roundN(r?.realLastPrice??r?.lastPrice,8),
      v6RealPrice1mPct:roundN(r?.realPrice1mPct??r?.price1mPct,3),
      v6RealPrice3mPct:roundN(r?.realPrice3mPct,3),
      v6RealPrice5mPct:roundN(r?.realPrice5mPct??r?.price5mPct,3),
      v6RejectReasons:reject,
      v6Momentum:roundN(momentum,3),
      v6Flow:roundN(flow,3),
      v6RealVolume:roundN(realVolume,3),
      v6RealVol1mX:roundN(r?.realVol1mX,2),
      v6RealVol3mX:roundN(r?.realVol3mX,2),
      v6RealVol5mX:roundN(r?.realVol5mX,2),
      ...
      reasons:[
        ...
        `Momentum ${roundN(momentum,2)} · flow ${roundN(flow,2)} · realVol ${roundN(realVolume,2)} · vol ${roundN(vol,2)} · OF ${roundN(orderFlow,2)} · OI ${roundN(oiScore,2)}`,
  ...
  data.explosionRanking=scored.map(x=>({
    symbol:x.symbol,direction:x.direction,score:x.v6ExplosionScore,earlyness:x.v6Earlyness,entryType:x.paperEntryType,
    expectedMoveR:x.v6ExpectedMoveR,oi5mPct:x.v6OiChange5mPct,oi15mPct:x.v6OiChange15mPct,
    flow:x.v6Flow,realVolume:x.v6RealVolume,realVol1mX:x.v6RealVol1mX,realVol3mX:x.v6RealVol3mX,realVol5mX:x.v6RealVol5mX,realLastPrice:x.v6RealLastPrice,pulseFresh:x.v6PulseFresh,radarFresh:x.v6RadarFresh,riskSource:x.v6RiskPlanSource,realPrice1mPct:x.v6RealPrice1mPct,realPrice3mPct:x.v6RealPrice3mPct,realPrice5mPct:x.v6RealPrice5mPct,momentum:x.v6Momentum,volatility:x.v6VolatilityExpansion,
    orderFlow:x.v6OrderFlow,book:x.v6BookImbalance,oi:x.v6OiScore,funding:x.v6FundingPressure,regime:x.v6MarketRegime,
    liquidationPressureProxy:x.v6LiquidationPressureProxy,paperEligible:x.v6PaperEligible,rejectReasons:x.v6RejectReasons
  }));
  data.version="BINGX_V6_EXPLOSIVE_MOVE_ENGINE_V62";
  data.v6={mode:"PAPER_TEST",liveOrders:false,objective:"explosive_move_30m_2h_6h",signals:data.signals.length};
  return data;
}

async function logV6Decisions(env,data){
  try{
    const rows=(Array.isArray(data?.all)?data.all:[]).slice(0,30).map((x,rank)=>({
      rank:rank+1,symbol:x.symbol,direction:x.direction,score:x.v6ExplosionScore,earlyness:x.v6Earlyness,entryType:x.paperEntryType,
      expectedMoveR:x.v6ExpectedMoveR,paperEligible:Boolean(x.v6PaperEligible),entryType:x.paperEntryType,standardEligible:Boolean(x.v6StandardEligible),ignitionEligible:Boolean(x.v6IgnitionEligible),preRank:x.v6PreRank,
      realVol1mX:x.v6RealVol1mX,realVol3mX:x.v6RealVol3mX,realVol5mX:x.v6RealVol5mX,
      realLastPrice:x.v6RealLastPrice,pulseFresh:x.v6PulseFresh,radarFresh:x.v6RadarFresh,riskSource:x.v6RiskPlanSource,
      realPrice1mPct:x.v6RealPrice1mPct,realPrice3mPct:x.v6RealPrice3mPct,realPrice5mPct:x.v6RealPrice5mPct,
      rejectReasons:x.v6RejectReasons||[],entry:x.entry,stop:x.stop,tp1:x.tp1,tp2:x.tp2
    }));
    await trackerStub(env).fetch("https://paper.local/v6-decision-log",{
      method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({scannedAt:data?.scannedAt,rows})
    });
  }catch{}
}
...
async function telegramDiscoverChatId(env){
  const token=String(env?.TELEGRAM_BOT_TOKEN||"").trim();
  if(!token)throw new Error("TELEGRAM_BOT_TOKEN runtime'da yok");
  const res=await fetch(`https://api.telegram.org/bot${token}/getUpdates`,{headers:{accept:"application/json"}});
  const text=await res.text();
  if(!res.ok)throw new Error(`Telegram getUpdates HTTP ${res.status} · ${text.slice(0,220)}`);
  let json;
  try{json=JSON.parse(text);}catch(_){throw new Error("Telegram getUpdates geçersiz JSON döndürdü");}
  const updates=Array.isArray(json?.result)?json.result:[];
  for(let i=updates.length-1;i>=0;i--){
    const chat=updates[i]?.message?.chat||updates[i]?.edited_message?.chat||updates[i]?.callback_query?.message?.chat;
    if(chat?.id!=null)return String(chat.id);
  }
  throw new Error("Telegram chat bulunamadı; bota /start veya normal bir mesaj gönder");
}
...
async function telegramSend(env,title,message){
  ...
  if(!token)throw new Error("TELEGRAM_BOT_TOKEN runtime'da yok");
  let chatId=String(env?.TELEGRAM_CHAT_ID||"").trim();
  if(!chatId)chatId=await telegramDiscoverChatId(env);

  const send=async(id)=>{
    const publicUrl=String(env?.PUBLIC_BASE_URL||"https://bingx-v6-paper.yasinaltas39.workers.dev").replace(/\/$/,"");
    const res=await fetch(`https://api.telegram.org/bot${token}/sendMessage`,{
      ...
      body:JSON.stringify({
        ...
        text:`📈 ${title}\n\n${message}`,
        disable_web_page_preview:true,
        reply_markup:{inline_keyboard:[[{text:"V6 panelini aç",url:publicUrl}]]}
      })
    });
  ...
  let out=await send(chatId);
  if(!out.res.ok && out.res.status===400 && /chat not found/i.test(out.detail)){
    const discovered=await telegramDiscoverChatId(env);
    if(discovered!==chatId){ chatId=discovered; out=await send(chatId); }
  }
  if(!out.res.ok)throw new Error(`Telegram HTTP ${out.res.status} · ${out.detail.slice(0,220)}`);
  return true;
}
...
async function sendPrimary(env,title,message){
  let telegramError=null, ntfyError=null;
  try{ if(await telegramSend(env,title,message))return "telegram"; }catch(e){ telegramError=e; console.error("TELEGRAM_ERROR",String(e?.message||e)); }
  try{ if(await ntfyFallback(env,title,message))return "ntfy"; }catch(e){ ntfyError=e; console.error("NTFY_ERROR",String(e?.message||e)); }
  if(telegramError)throw telegramError;
  if(ntfyError)throw ntfyError;
  throw new Error("Bildirim kanalı kullanılamıyor: Telegram/ntfy runtime değişkenleri yok");
}

function signalMessage(x){
  return [
    `${x.symbol} ${x.direction} · ${x.paperEntryType==="V6_IGNITION"?"V6 IGNITION":"V6 EXPLOSIVE MOVE"}`,
...
function paperSummaryHtml(paper){
  ...
  const fresh=trades.filter(t=>/^V6\./.test(String(t.scannerVersion||"")));
...
function openPaperHtml(paper){
  ...
  return trades.map(t=>{
    ...
    const meta=/^V6\./.test(String(t.scannerVersion||""))?` · Explosion ${esc(t.v6ExplosionScore??t.executionScore??"-")}/10 · Early ${esc(t.v6Earlyness??"-")} · Exp ${esc(t.v6ExpectedMoveR??"-")}R`:"";
...
class PaperTracker extends BasePaperTracker {
  async ensureV219Migration(){
    ...
    const m={version:"V6.1-core-migration",archivedAt,archivedTrades:oldTrades.length};
  ...
  async updateV6ForwardLabels(rows,leaders,ts){
    ...
    for(const x of top){
      ...
      samples.push({
        ...
        realVol1mX:x.realVol1mX,realVol3mX:x.realVol3mX,realVol5mX:x.realVol5mX,
  ...
  async getV6ForwardStats(){
    const raw=(await this.state.storage.get("v6_forward_labels"))||[];
    ...
    const r=(v,d=3)=>finite(v)?Number(Number(v).toFixed(d)):null;
    const summarize=(arr,h)=>{
      const rows=arr.filter(s=>s?.horizons?.[String(h)]&&finite(s.horizons[String(h)].returnPct));
      const rets=rows.map(s=>Number(s.horizons[String(h)].returnPct));
      const mfes=rows.filter(s=>finite(s.horizons[String(h)].mfePct)).map(s=>Number(s.horizons[String(h)].mfePct));
      const maes=rows.filter(s=>finite(s.horizons[String(h)].maePct)).map(s=>Number(s.horizons[String(h)].maePct));
      const wins=rets.filter(x=>x>0).length,losses=rets.filter(x=>x<0).length,flat=rets.length-wins-losses;
      return {n:rows.length,winRatePct:r(pct(wins,rows.length),2),wins,losses,flat,
        directionalReturnPct:{avg:r(avg(rets)),median:r(med(rets)),best:r(rets.length?Math.max(...rets):null),worst:r(rets.length?Math.min(...rets):null)},
        mfePct:{avg:r(avg(mfes)),median:r(med(mfes)),best:r(mfes.length?Math.max(...mfes):null)},
        maePct:{avg:r(avg(maes)),median:r(med(maes)),worst:r(maes.length?Math.min(...maes):null)}
      };
    };
    const group360=(name,test)=>({name,...summarize(samples.filter(test),360)});
    const scoreBands=[
      ["score<6",s=>finite(s.preRankScore)&&Number(s.preRankScore)<6],
      ["6<=score<7",s=>finite(s.preRankScore)&&Number(s.preRankScore)>=6&&Number(s.preRankScore)<7],
      ["7<=score<8",s=>finite(s.preRankScore)&&Number(s.preRankScore)>=7&&Number(s.preRankScore)<8],
      ["8<=score<9",s=>finite(s.preRankScore)&&Number(s.preRankScore)>=8&&Number(s.preRankScore)<9],
      ["score>=9",s=>finite(s.preRankScore)&&Number(s.preRankScore)>=9]
    ].map(([name,test])=>group360(name,test));
    const signalGroups=[
      ["flowPaceX>=1.5",s=>finite(s.flowPaceX)&&Number(s.flowPaceX)>=1.5],
      ["flowPaceX>=2",s=>finite(s.flowPaceX)&&Number(s.flowPaceX)>=2],
      ["flowAccelX>=1.75",s=>finite(s.flowAccelX)&&Number(s.flowAccelX)>=1.75],
      ["flowAccelX>=2.5",s=>finite(s.flowAccelX)&&Number(s.flowAccelX)>=2.5],
      ["volExpansionX>=1.4",s=>finite(s.volExpansionX)&&Number(s.volExpansionX)>=1.4],
      ["volExpansionX>=2",s=>finite(s.volExpansionX)&&Number(s.volExpansionX)>=2],
      ["bookAligned>=60/<=40",s=>finite(s.bookBuyPct)&&((s.direction==="LONG"&&Number(s.bookBuyPct)>=60)||(s.direction==="SHORT"&&Number(s.bookBuyPct)<=40))],
      ["score>=8 + accel>=2 + vol>=1.4",s=>Number(s.preRankScore)>=8&&Number(s.flowAccelX)>=2&&Number(s.volExpansionX)>=1.4],
      ["score>=8 + pace>=1.5 + bookAligned",s=>Number(s.preRankScore)>=8&&Number(s.flowPaceX)>=1.5&&finite(s.bookBuyPct)&&((s.direction==="LONG"&&Number(s.bookBuyPct)>=60)||(s.direction==="SHORT"&&Number(s.bookBuyPct)<=40))]
    ].map(([name,test])=>group360(name,test));
    return {
      ok:true,generatedAt:new Date().toISOString(),count:samples.length,
      completed:{30:samples.filter(s=>s?.horizons?.["30"]).length,120:samples.filter(s=>s?.horizons?.["120"]).length,360:samples.filter(s=>s?.horizons?.["360"]).length},
      overall:{30:summarize(samples,30),120:summarize(samples,120),360:summarize(samples,360)},
      byDirection360:{LONG:summarize(samples.filter(s=>s.direction==="LONG"),360),SHORT:summarize(samples.filter(s=>s.direction==="SHORT"),360)},
      scoreBands360:scoreBands,signalGroups360:signalGroups,
      note:"directionalReturnPct is already signed in the candidate direction; positive is favorable. MFE/MAE are forward excursion labels. Stats are descriptive PAPER/TEST diagnostics, not live execution rules."
    };
  }
  ...
  async updateV6DecisionLog(rows,scannedAt){
    ...
    for(const r of (Array.isArray(rows)?rows:[]).filter(x=>Number(x?.score)>=4.5).slice(0,20)){
      const sym=String(r?.symbol||""); if(!sym)continue;
      const snap={...r,at:iso};
      const prev=state[sym]||null;
      state[sym]={
        symbol:sym,firstAt:prev?.firstAt||iso,lastAt:iso,
        maxScore:Math.max(Number(prev?.maxScore||0),Number(r?.score||0)),
        maxDecision:!prev||Number(r?.score)>=Number(prev?.maxScore||0)?snap:prev.maxDecision,
        lastDecision:snap
      };
    }
    for(const [sym,v] of Object.entries(state))if(ts-Date.parse(v?.lastAt||0)>12*60*60*1000)delete state[sym];
    const kept=Object.values(state).sort((a,b)=>Date.parse(b.lastAt)-Date.parse(a.lastAt)).slice(0,120);
    state=Object.fromEntries(kept.map(x=>[x.symbol,x]));
    await this.state.storage.put("v6_decision_watch",state);
    return {ok:true,count:kept.length,latest:kept.slice(0,20)};
  }

  async getV6DecisionLog(){
    const state=(await this.state.storage.get("v6_decision_watch"))||{};
    const candidates=Object.values(state||{}).sort((a,b)=>Date.parse(b.lastAt)-Date.parse(a.lastAt));
  ...
  async notifyClosed(closedNow){
    ...
    for(const t of closedNow){
      if(!/^V6\./.test(String(t?.scannerVersion||""))){
  ...
  async fetch(request){
    ...
    if(url.pathname==="/v32-volume-radar"){
      ...
      if(request.method==="POST"){
        ...
        if(body?.enrichOnly&&Array.isArray(body?.leaders)){
          leaderState={...leaderState,updatedAt:ts,leaders:body.leaders.slice(0,V32_RADAR.maxLeaders),universeCount:Number(body?.universeCount||leaderState?.universeCount||0),warmed:true};
          await this.state.storage.put("v32_volume_leaders",leaderState);
          return Response.json({ok:true,...leaderState});
        }
