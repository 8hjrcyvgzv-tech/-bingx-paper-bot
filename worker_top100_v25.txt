...
async function scan(){
  ...
  return {
    mode:"PAPER_SCAN",version:"TOP100_V2_5",scannedAt:new Date().toISOString(),btcDirection:btcDir,minScore:CFG.minScore,
...
async function sendNtfy(env,data){
  ...
  for(const x of data.signals){
    ...
    const payload={topic:env.NTFY_TOPIC,title:`BingX ${x.direction} ${x.score}/10 · V2.5`,message,priority:4,tags:["chart_with_upwards_trend"]};
    if(token){
...
function page(data,execMode="KAPALI"){
  ...
  <div class="head"><h2>BingX Paper Bot · Top100 V2.5</h2><div>Tarama: PAPER · İşlem köprüsü: ${execMode} · Eşik: ${data.minScore}/10 · BTC: ${data.btcDirection}</div>
