...
async function bingxJson(url) {
  ...
  if (json?.code != null && Number(json.code) !== 0) {
    throw new Error(`BingX: ${json.msg || json.code}`);
  }
  return json;
}
...
function analyze(symbol, h1, m15, btcDir, quoteVolume = null) {
  ...
  return {
    ...
    quoteVolume24h: quoteVolume == null ? null : Math.round(quoteVolume),
    ...
    paperMarginUSDT: +(CFG.paperBalance * CFG.maxMarginPct).toFixed(2),
    reasons,
  };
}

function currentShardIndex(date = new Date()) {
  return Math.floor((date.getUTCMinutes() % 15) / 3); // 0..4 for */3 cron
}

async function scan(shardIndex = currentShardIndex()) {
  const [top, btc1h, btc15] = await Promise.all([
  ...
  const shardCount = Math.ceil(top.length / CFG.shardSize);
  const safeShard = shardCount ? shardIndex % shardCount : 0;
  const start = safeShard * CFG.shardSize;
  const selected = top.slice(start, start + CFG.shardSize);
  const results = [];

  // 3 coins at a time = max 6 simultaneous kline connections.
      ...
      try {
        ...
        if (item.symbol === "BTC-USDT") {
          ...
          [h1, m15] = await Promise.all([
            getKlines(item.symbol, "1h"),
            getKlines(item.symbol, "15m"),
          ]);
        }
        return analyze(item.symbol, h1, m15, btcDir, item.quoteVolume);
      } catch (e) {
        return { symbol: item.symbol, error: String(e.message || e) };
      }
    }));
  ...
  return {
    ...
    universe: `BingX top ${top.length} USDT perpetual by 24h quote volume`,
    shard: safeShard + 1,
    shardCount,
    scannedSymbols: selected.map(x => x.symbol),
...
async function sendNtfy(env, data) {
  ...
  const body = data.signals.map(x => [
    ...
    `TP1: ${x.tp1 ?? "-"}`,
    `TP2: ${x.tp2 ?? "-"}`,
    `RSI: ${x.rsi ?? "-"} | Hacim: ${x.volumeRatio ?? "-"}x | ATR: ${x.atrPct ?? "-"}%`,
  ].join("\n")).join("\n\n");
  ...
  const res = await fetch(`https://ntfy.sh/${encodeURIComponent(env.NTFY_TOPIC)}`, {
    ...
    headers: {
      ...
      title: "BingX 7/10+ Sinyal · Top100",
      priority: "high",
...
export default {
  ...
  async scheduled(controller, env, ctx) {
    const data = await scan(currentShardIndex());
