...
async function getDailyKlines(symbol,limit=V220_BELIT.dailyLookback){
...
export function analyzeBelitDaily(rows,x){
  ...
  if(!current||direction==="NEUTRAL")return {stage:"NONE",setupQuality:0,entryQuality:num(x?.score)||0,watchCandidate:false,extended:false};
  const a=atr(completed,20),adr=adrPct(completed,20);
  if(!(a>0))return {stage:"NONE",setupQuality:0,entryQuality:num(x?.score)||0,watchCandidate:false,extended:false};
  ...
  if(!boundary)return {
    stage:"NONE",setupQuality:round2(Math.min(6,(num(x?.score)||0)*0.65)),entryQuality:num(x?.score)||0,watchCandidate:false,extended:false,
    boundary:null,boundaryTests:0,adr20Pct:round2(adr),atr20Pct:round2(a/current.close*100),compressionRatio:round2(comp.ratio),
  ...
  let setup=0;
  setup+=boundary.tests>=4?2.25:boundary.tests===3?2:1.5;
  setup+=Math.abs(distPct)<=2.5?1.5:Math.abs(distPct)<=5?1:0;
  ...
  setup+=(num(x?.volumeRatio)||0)>=1.2?1:(num(x?.volumeRatio)||0)>=1.05?0.5:0;
  setup+=pressure!=null&&pressure>=60?0.75:pressure!=null&&pressure>=55?0.5:0;
  if(x?.riskOk&&x?.rangeOk)setup+=0.5;
  if(["BREAKOUT_CONFIRMED","RETEST","CONTINUATION_BREAKOUT"].includes(stage))setup+=0.75;
  setup=clamp(Math.round(setup*4)/4,0,10);

  let entry=num(x?.score)||0;
  if(stage==="PRE_BREAKOUT")entry=Math.min(4.5,3.25+(setup-7)*0.4);
  else if(stage==="CONTINUATION_PREP")entry=Math.min(5,3.75+(setup-7)*0.45);
  else if(stage==="BREAKOUT_CONFIRMED")entry=7.25+0.75+(num(x?.volumeRatio)>=1.2?0.75:0)+(pressure>=55?0.5:0)+(x?.riskOk&&x?.rangeOk?0.5:0);
  else if(stage==="RETEST")entry=7.5+(comp.ok?0.5:0)+(pressure>=55?0.5:0)+(x?.riskOk&&x?.rangeOk?0.5:0);
  else if(stage==="CONTINUATION_BREAKOUT")entry=8+(num(x?.volumeRatio)>=1.2?0.75:0)+(pressure>=55?0.5:0)+(x?.riskOk&&x?.rangeOk?0.5:0);
  else if(stage==="EXTENDED")entry=Math.min(3,Math.max(1,(num(x?.score)||0)-5));
  entry=clamp(Math.round(entry*4)/4,0,10);

  const watchCandidate=["PRE_BREAKOUT","CONTINUATION_PREP"].includes(stage)&&setup>=V220_BELIT.minWatchSetupQuality&&
    x?.trend4h===direction&&x?.trend1h===direction&&Boolean(x?.ema200Aligned)&&!isSyntheticSymbol(x?.symbol);

  return {
    stage,setupQuality:setup,entryQuality:entry,watchCandidate,extended,boundary:+level.toFixed(8),boundaryTests:boundary.tests,
    distanceToBoundaryPct:round2(distPct),distanceFromBoundaryATR:round2(distAtr),barsSinceBreak:since,persistentBreakout:persistent,
    compressionRatio:round2(comp.ratio),compressionTightPct:round2(comp.tightPct),directionalSqueeze:squeeze,adr20Pct:round2(adr),
    atr20Pct:round2(a/current.close*100),smaDaily:smas.label,smaStackScore:round2(smas.score),isCore5:CORE5.has(String(x?.symbol||"").toUpperCase())
  };
}

export function isV220Signal(x){
  return Boolean(x?.v219Qualifies)&&!Boolean(x?.belitExtended)&&(num(x?.entryQuality)==null||Number(x.entryQuality)>=V220_BELIT.minEntryQuality);
}
...
export async function enrichBelitData(raw){
  ...
  for(let i=0;i<source.length;i+=2){
    ...
    const enriched=await Promise.all(batch.map(async x=>{
      if(x?.error)return x;
      let b={stage:"NONE",setupQuality:0,entryQuality:num(x?.score)||0,watchCandidate:false,extended:false,isCore5:CORE5.has(String(x?.symbol||"").toUpperCase())};
      let belitError=null;
      try{ b=analyzeBelitDaily(await getDailyKlines(x.symbol),x); }catch(e){ belitError=String(e?.message||e); }
      const out={
        ...x,
        belitStage:b.stage,setupQuality:b.setupQuality,entryQuality:b.entryQuality,belitWatchCandidate:b.watchCandidate,belitExtended:b.extended,
        boundary:b.boundary??null,boundaryTests:b.boundaryTests??0,distanceToBoundaryPct:b.distanceToBoundaryPct??null,
        ...
        adr20Pct:b.adr20Pct??null,atr20DailyPct:b.atr20Pct??null,smaDaily:b.smaDaily??null,smaStackScore:b.smaStackScore??null,
        isCore5:b.isCore5??CORE5.has(String(x?.symbol||"").toUpperCase()),belitError
      };
      out.v220Qualifies=isV220Signal(out);
      out.qualifies=out.v220Qualifies;
      return out;
    }));
    all.push(...enriched);
  }
  const signals=all.filter(isV220Signal)
    ...
    .slice(0,V220_BELIT.maxSignalsPerScan);
  ...
  const watch=all.filter(x=>x.belitWatchCandidate&&!signalSymbols.has(x.symbol))
    .sort((a,b)=>(Number(b.setupQuality)||0)-(Number(a.setupQuality)||0)||Math.abs(Number(a.distanceToBoundaryPct)||99)-Math.abs(Number(b.distanceToBoundaryPct)||99))
    .slice(0,V220_BELIT.maxWatchPerScan);
  return {...raw,version:"TOP100_V2_20",signals,watch,all};
}
