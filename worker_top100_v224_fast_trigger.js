import baseWorker, { PaperTracker as BasePaperTracker } from "./worker_top100_v224_fast_base.js";
import { enrichBelitData, isV223Signal, stageLabel, publicStatusLabel } from "./belit_daily_v224.js";
...
function upgradeData(raw){
  ...
  return {...raw,version:"TOP100_V2_24_FAST",minScore:V219.minScore,signals,all};
}
...
async function scanThroughBase(env,focusSymbols=[]){
  ...
  const data=await enrichBelitData(upgradeData(await res.json()));
...
async function updateFastWatch(env,data){
  try{
    const candidates=(Array.isArray(data?.all)?data.all:[]).filter(x=>x?.belitWatchCandidate);
    const res=await trackerStub(env).fetch("https://paper.local/v223-fast-watch",{
      method:"POST",headers:{"content-type":"application/json"},
...
function signalMessage(x){
  return [
    ...
    `15dk hızlı tetik: ${x.fastTriggerDistanceATR??"-"} ATR · kırılım yaşı ${x.intradayBreakAgeBars??"-"} mum`,
    `SL: ${x.stop??"-"} (${x.stopPct??"-"}%)`,
    `TP1: ${x.tp1??"-"} · TP2: ${x.tp2??"-"}`,
    ...
    `4s/1s/15dk: ${x.trend4h}/${x.trend1h}/${x.trend15m} · ADX4s ${x.adx4h??"-"} · Hacim ${x.volumeRatio??"-"}x · Funding ${x.fundingRate??"-"}`,
    `Teorik margin: ${marginForSignal(x)??"-"} USDT · 5x isolated`,
...
function paperSummaryHtml(paper){
  ...
  const fresh=trades.filter(t=>t.scannerVersion==="V2.24");
  ...
  return `V2.24 YENİ: <b>${freshOpen.length} açık</b> · ${freshClosed.length} kapanan · Win %${freshWr.toFixed(1)} · <span class="muted">Geçmiş kayıt: ${totalOpen} açık / ${totalClosed} kapanan</span>`;
}
...
function openPaperHtml(paper){
  ...
  return trades.map(t=>{
    ...
    const meta=t.scannerVersion==="V2.24"?` · Setup ${esc(t.setupQuality??"-")}/10 · Execution ${esc(t.executionScore??t.entryQuality??"-")}/10 · ${esc(publicStatusLabel(t.publicStatus||"WAIT"))}`:"";
...
export class PaperTracker extends BasePaperTracker {
  async ensureV219Migration(){
    ...
    const m={version:"V2.24",archivedAt,archivedTrades:oldTrades.length};
  ...
  async addSignals(trades,signals,scannedAt){
    ...
    const selected=[];
    const sorted=(Array.isArray(signals)?signals:[]).filter(isV223Signal).sort((a,b)=>(Number(b.executionScore)||0)-(Number(a.executionScore)||0)||(Number(b.setupQuality)||0)-(Number(a.setupQuality)||0)||(Number(b.score)||0)-(Number(a.score)||0));
  ...
  async fetch(request){
    ...
    if(url.pathname==="/v223-fast-watch"){
      ...
      if(request.method==="POST"){
        ...
        const candidateMap=new Map(candidates.filter(x=>x?.symbol&&x?.belitWatchCandidate).map(x=>[String(x.symbol),x]));
...
export default {
  async fetch(request,env){
    ...
    if(url.pathname==="/notify-test"){
      ...
      try{const channel=await sendPrimary(env,"BingX BİLDİRİM TESTİ · V2.24","Bildirim kanalı çalışıyor. V2.24; Telegram yalnız tetiklenmiş LONG/SHORT sinyallerini gönderir. Worker EXECUTION_MODE zorla TEST kilitlidir. HAZIRLANIYOR/ARMED erken uyarıları sadece web radarında kalır. Setup/Tetik/Execution ayrımı ve 3 test kabul / 4+ premium boundary filtresi aktiftir.");return Response.json({ok:true,channel},{headers:{"cache-control":"no-store"}});}catch(e){return Response.json({ok:false,error:String(e?.message||e)},{status:500});}
    }
  ...
  async scheduled(controller,env,ctx){
    ...
    // Telegram is intentionally signal-only in V2.24.
    ...
    console.log(JSON.stringify({cron:controller.cron,version:"V2.24",scannedAt:data.scannedAt,shard:data.shard,scanned:data.all?.length||0,qualified:data.signals?.length||0,watch:data.watch?.length||0,fastWatch:(fastWatch?.watch||focus).map(x=>x.symbol),alerts:alerts.length,telegramWatchAlerts:0,paper:paper?.stats||null}));
    if(alerts.length){
      ctx.waitUntil((async()=>{
