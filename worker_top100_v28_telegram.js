...
async function ntfyFallback(env,title,message){
  ...
  for(let attempt=0;attempt<4;attempt++){
    const res=await fetch(`https://ntfy.sh/${encodeURIComponent(topic)}`,{
      method:"POST",
      headers:{"content-type":"text/plain; charset=utf-8",title,priority:"high",tags:"chart_with_upwards_trend"},
      body:message
    });
...
export default {
  async fetch(request,env){
    ...
    if(ct.includes("text/html")){
      const html=(await res.text()).replaceAll("V2.7","V2.8");
    ...
    if(data.signals?.length){
      ctx.waitUntil((async()=>{
        for(const x of data.signals){
          try{
            await sendPrimary(env,`BingX ${x.direction} ${x.score}/10 · V2.8`,signalMessage(x));
          }catch(e){
