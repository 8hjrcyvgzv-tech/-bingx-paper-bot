// BingX Paper Bot V6.1 — Explosive Move Engine · Missed-Explosion / ARB fix
...
function v6RiskPlan(x,direction,explosionScore){
  const entry=Number(x?.lastPrice??x?.price??x?.entry);
  ...
  const structuralRisk=Number.isFinite(recentExtreme)
    ...
    : 0;
  const volatilityRisk=Number.isFinite(atrAbs)?0.70*atrAbs:entry*0.008;
  const riskDist=Math.max(entry*0.0045,volatilityRisk,structuralRisk>0?structuralRisk+(Number.isFinite(atrAbs)?0.10*atrAbs:0):0);
  const stopPct=100*riskDist/entry;
  if(!(riskDist>0)||stopPct>4.0)return {valid:false,reason:`stop ${stopPct.toFixed(2)}%`,entry,stopPct};
  ...
  return {
    valid:true,source:"V6_DYNAMIC_15M_ATR",entry,stop,tp1,tp2,targetR,stopPct,
...
function expectedMoveR(x,explosionScore,riskPlan,r){
  ...
  const p5=Math.abs(Number(r?.realPrice5mPct??r?.price5mPct));
  ...
  const candidates=[
    ...
    Number.isFinite(p5)?p5*(1.25+0.08*Math.min(5,rv)):null
...
async function applyV6ExplosionEngine(data,radarLeaders,env){
  ...
  const scored=all.map(x=>{
    ...
    const riskPlan=v6RiskPlan(x,direction,score);
    const moveR=expectedMoveR(x,score,riskPlan,r);
    const margin=v6Margin(score,moveR,early);
    // V6.1: legacy trend/Belit/range gates no longer veto an explosion.
    // The hard safety gate is now a direction-specific V6 stop plan + fresh entry/mark validity.
    const safeRiskPlan=Boolean(riskPlan?.valid)&&x?.entryStillValid!==false;
    ...
    if(x?.entryStillValid===false)reject.push("giriş/mark güncelliği geçersiz");
    if(score<V6.minPaperScore)reject.push(`explosion<${V6.minPaperScore}`);
    if(!(Number(moveR)>=V6.minExpectedMoveR))reject.push(`beklenen hareket<${V6.minExpectedMoveR}R`);
  ...
  data.explosionRanking=scored.map(x=>({
    ...
    flow:x.v6Flow,realVolume:x.v6RealVolume,realVol1mX:x.v6RealVol1mX,realVol3mX:x.v6RealVol3mX,realVol5mX:x.v6RealVol5mX,momentum:x.v6Momentum,volatility:x.v6VolatilityExpansion,
  ...
  data.version="BINGX_V6_EXPLOSIVE_MOVE_ENGINE_V61";
...
async function logV6Decisions(env,data){
  try{
    const rows=(Array.isArray(data?.all)?data.all:[]).slice(0,30).map((x,rank)=>({
      rank:rank+1,symbol:x.symbol,direction:x.direction,score:x.v6ExplosionScore,earlyness:x.v6Earlyness,
      expectedMoveR:x.v6ExpectedMoveR,paperEligible:Boolean(x.v6PaperEligible),preRank:x.v6PreRank,
