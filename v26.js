...
async function scan(){
  ...
  return {
    mode:"PAPER_SCAN",version:"TOP100_V2_6",scannedAt:new Date().toISOString(),btcDirection:btcDir,minScore:CFG.minScore,
...
async function ntfyFetch(url,options){
  let lastStatus=0;
  for(let attempt=0;attempt<4;attempt++){
    const res=await fetch(url,options);
    if(res.ok)return res;
    lastStatus=res.status;
    if(res.status!==429)throw new Error(`ntfy gönderim hatası: HTTP ${res.status}`);
    // ntfy varsayılan istek kovası yaklaşık 5 sn'de bir yenilenir.
    // 429 durumunda artan bekleme ile otomatik tekrar dene.
    await sleep(6000*(attempt+1));
  }
  throw new Error(`ntfy gönderim hatası: HTTP ${lastStatus||429} (tekrar denemeler başarısız)`);
}

async function notifyText(env,title,message,priority="high"){
  if(!env?.NTFY_TOPIC)throw new Error("NTFY_TOPIC eksik");
  await ntfyFetch(`https://ntfy.sh/${encodeURIComponent(env.NTFY_TOPIC)}`,{
    method:"POST",
...
async function sendNtfy(env,data){
  ...
  for(const x of data.signals){
    ...
    const payload={topic:env.NTFY_TOPIC,title:`BingX ${x.direction} ${x.score}/10 · V2.6`,message,priority:4,tags:["chart_with_upwards_trend"]};
    if(token){
    ...
    await ntfyFetch("https://ntfy.sh",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(payload)});
    // Birden fazla güçlü sinyal aynı taramada çıkarsa ntfy'ye ani burst yapma.
    await sleep(5500);
  }
}
...
function page(data,execMode="KAPALI"){
  ...
  <div class="head"><h2>BingX Paper Bot · Top100 V2.6</h2><div>Tarama: PAPER · İşlem köprüsü: ${execMode} · Eşik: ${data.minScore}/10 · BTC: ${data.btcDirection}</div>
  <div>4s + 1s + 15dk · 5x isolated · 20 USDT margin · max 3 pozisyon · tek tuş onay</div><button class="notifyBtn" onclick="testNotify(this)">BİLDİRİM TESTİ</button>
  ...
  async function testNotify(btn){
    ...
    try{
      ...
      alert("ntfy test bildirimi gönderildi");
...
export default {
  async fetch(request,env){
    ...
    if(url.pathname==="/notify-test"){
      ...
      try{
        await notifyText(env,"BingX BİLDİRİM TESTİ","Bildirim kanalı çalışıyor. 7/10+ sinyaller ntfy üzerinden gönderilecek.","high");
  ...
  async scheduled(controller,env,ctx){
    ...
    if(data.signals.length)ctx.waitUntil(sendNtfy(env,data).catch(e=>console.error("NTFY_ERROR",String(e?.message||e))));
  },
};
