import baseWorker, { PaperTracker as BasePaperTracker } from "./worker_top100_v223_fast_base.js";
import { enrichBelitData, isV223Signal, stageLabel, publicStatusLabel } from "./belit_daily_v223.js";
...
function upgradeData(raw){
  ...
  return {...raw,version:"TOP100_V2_23_FAST",minScore:V219.minScore,signals,all};
}
...
function paperSummaryHtml(paper){
  ...
  return `AUTO PAPER V2.23: <b>${open} açık</b> · ${closed} kapanan · Win %${wr.toFixed(1)} · Açık toplam <span class="${pnl>0?"good":pnl<0?"bad":"muted"}">${pnl>=0?"+":"-"}$${Math.abs(pnl).toFixed(2)}</span>`;
}
...
function openPaperHtml(paper){
  ...
  return trades.map(t=>{
    ...
    const meta=t.scannerVersion==="V2.23"?` · Setup ${esc(t.setupQuality??"-")}/10 · Execution ${esc(t.executionScore??t.entryQuality??"-")}/10 · ${esc(publicStatusLabel(t.publicStatus||"WAIT"))}`:"";
...
export class PaperTracker extends BasePaperTracker {
  async ensureV219Migration(){
    ...
    const m={version:"V2.23",archivedAt,archivedTrades:oldTrades.length};
...
export default {
  ...
  async scheduled(controller,env,ctx){
    ...
    // Telegram is intentionally signal-only in V2.23.
    ...
    console.log(JSON.stringify({cron:controller.cron,version:"V2.23",scannedAt:data.scannedAt,shard:data.shard,scanned:data.all?.length||0,qualified:data.signals?.length||0,watch:data.watch?.length||0,fastWatch:(fastWatch?.watch||focus).map(x=>x.symbol),alerts:alerts.length,telegramWatchAlerts:0,paper:paper?.stats||null}));
    if(alerts.length){
      ctx.waitUntil((async()=>{
