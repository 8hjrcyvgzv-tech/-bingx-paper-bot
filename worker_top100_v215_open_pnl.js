...
async function scan(){
  ...
  return {
    mode:"PAPER_SCAN",version:"TOP100_V2_15",scannedAt:new Date().toISOString(),btcDirection:btcDir,minScore:CFG.minScore,
...
function openPaperMetrics(trade){
  ...
  if(!(price>0))return {
    currentPrice:null,unrealizedPct:null,unrealizedR:null,gross5xPct:null
  };
  ...
  return {
    ...
    gross5xPct:+(signedPct*CFG.leverage).toFixed(2)
  };
}
...
async function sendSignalAlerts(env,data){
  ...
  for(const x of data.signals){
    ...
    try{
      await sendPrimary(env,`BingX ${x.direction} ${x.score}/10 · V2.15`,message,actionUrl,actionLabel);
    }catch(e){
...
function page(data,execMode="KAPALI",paper=null){
  ...
  const openCards=openPaper.length?openPaper.map(t=>{
    ...
    const pnlCls=r==null?"muted":r>0?"good":r<0?"bad":"muted";
    return `<div class="paperCard">
      <div class="row"><b>${t.symbol} · ${t.direction} · ${t.score}/10</b><span class="${pnlCls}">${r==null?"fiyat bekleniyor":`${r>=0?"+":""}${r}R`}</span></div>
      <div>Giriş: ${t.entry} · Güncel: ${t.currentPrice??"-"}</div>
      <div class="${pnlCls}">Anlık: ${pct==null?"-":`${pct>=0?"+":""}${pct}%`} · 5x brüt: ${gross==null?"-":`${gross>=0?"+":""}${gross}%`}</div>
  ...
  <div class="head"><h2>BingX Paper Bot · Top100 V2.15</h2><div>Tarama: PAPER · İşlem köprüsü: ${execMode} · Eşik: ${data.minScore}/10 · BTC: ${data.btcDirection}</div>
  <div>4s + 1s + 15dk · 5x isolated · 20 USDT margin · LIVE max 5 pozisyon · TEST=AUTO PAPER</div>
  <div>${paper?.stats?`AUTO PAPER: ${paper.stats.open} açık · ${paper.stats.closed} kapanan · Win %${paper.stats.winRatePct} · Kapalı Ort. ${paper.stats.avgR}R · Açık: ${paper.stats.openWins} kâr / ${paper.stats.openLosses} zarar · Açık toplam ${paper.stats.totalOpenR>=0?"+":""}${paper.stats.totalOpenR}R`:"AUTO PAPER: başlatılıyor..."}</div>
