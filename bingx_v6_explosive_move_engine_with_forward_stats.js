...
class PaperTracker extends BasePaperTracker {
  ...
  async getV6ForwardStats(){
    ...
    const summarize=(rows,h)=>{
      ...
      return {
        samples:done.length,
        avgReturnPct:avg(vals.map(x=>x.returnPct)),
        avgMfePct:avg(vals.map(x=>x.mfePct)),
        avgMaePct:avg(vals.map(x=>x.maePct)),
        mfe1HitPct:pct(vals.filter(x=>Number(x.mfePct)>=1).length,vals.length),
        mfe2HitPct:pct(vals.filter(x=>Number(x.mfePct)>=2).length,vals.length),
        mfe3HitPct:pct(vals.filter(x=>Number(x.mfePct)>=3).length,vals.length),
        positiveReturnPct:pct(vals.filter(x=>Number(x.returnPct)>0).length,vals.length),
        return1Pct:pct(vals.filter(x=>Number(x.returnPct)>=1).length,vals.length),
        mae1WorsePct:pct(vals.filter(x=>Number(x.maePct)<=-1).length,vals.length),
        mae2WorsePct:pct(vals.filter(x=>Number(x.maePct)<=-2).length,vals.length)
      };
    };
    const byScoreBand={};
    for(const b of bands){
      const rows=arr.filter(x=>{
        const s=Number(x?.preRankScore);
        return Number.isFinite(s)&&s>=b.min&&s<b.max;
      });
      byScoreBand[b.name]={
        total:rows.length,
        LONG:{
          total:rows.filter(x=>x.direction==="LONG").length,
          horizons:Object.fromEntries(horizons.map(h=>[h,summarize(rows.filter(x=>x.direction==="LONG"),h)]))
        },
        SHORT:{
          total:rows.filter(x=>x.direction==="SHORT").length,
          horizons:Object.fromEntries(horizons.map(h=>[h,summarize(rows.filter(x=>x.direction==="SHORT"),h)]))
        },
        ALL:{
          horizons:Object.fromEntries(horizons.map(h=>[h,summarize(rows,h)]))
        }
      };
    }
    return {
      ...
      note:"preRankScore radar on-siralama skorudur; V6 Explosion Score degildir.",
      completed:{
