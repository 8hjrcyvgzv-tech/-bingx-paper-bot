...
async function scan(){
  ...
  return {
    mode:"PAPER_ONLY",version:"TOP100_V2_1",scannedAt:new Date().toISOString(),btcDirection:btcDir,minScore:CFG.minScore,
...
async function sendNtfy(env,data){
  ...
  const res=await fetch(`https://ntfy.sh/${encodeURIComponent(env.NTFY_TOPIC)}`,{
    method:"POST",
    headers:{"content-type":"text/plain; charset=utf-8",title:"BingX 7/10+ Sinyal · Top100 V2.1",priority:"high",tags:"chart_with_upwards_trend"},
    body,
  });
...
function page(data){
  ...
  <div class="head"><h2>BingX Paper Bot · Top100 V2.1</h2><div>Mod: PAPER ONLY · Eşik: ${data.minScore}/10 · BTC: ${data.btcDirection}</div>
  <div>4s + 1s + 15dk · Hacim + kırılım + EMA200 + ADX + Alış/Satış baskısı*</div>
