...
async function scan(){
  ...
  return {
    mode:"PAPER_SCAN",version:"TOP100_V2_16",scannedAt:new Date().toISOString(),btcDirection:btcDir,minScore:CFG.minScore,
...
async function sendSignalAlerts(env,data){
  ...
  for(const x of data.signals){
    ...
    try{
      await sendPrimary(env,`BingX ${x.direction} ${x.score}/10 · V2.16`,message,actionUrl,actionLabel);
    }catch(e){
...
function page(data,execMode="KAPALI",paper=null){
  ...
  <div class="head"><h2>BingX Paper Bot · Top100 V2.16</h2><div>Tarama: PAPER · İşlem köprüsü: ${execMode} · Eşik: ${data.minScore}/10 · BTC: ${data.btcDirection}</div>
